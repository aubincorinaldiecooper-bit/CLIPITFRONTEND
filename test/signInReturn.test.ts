...
beforeEach(() => {
  vi.clearAllMocks()
  window.history.replaceState(null, "", "/start?video=v1&then=publish%3Ac1")
})
...
describe("returnAddress", () => {
  it("carries the guest's hand-over, keeping the video and errand already parked on the address", async () => {
    ...
    expect(await returnAddress("a@b.c")).toBe("/start?video=v1&then=publish%3Ac1&handoff=tok_abc-_1")
  ...
  it("is the plain address when there is nothing to hand over — signed in already, or the API unreachable", async () => {
    ...
    expect(await returnAddress("a@b.c")).toBe("/start?video=v1&then=publish%3Ac1")
  })
